...
import           Control.Monad.Reply
import           Control.Monad.Reply.IO
...
main :: IO ()
main = withTerminal (runTerminalT $ runReplyT repl 0) >>= Prelude.print
...
repl :: (MonadTerminal m, MonadColorPrinter m, MonadMask m, MonadIO m) => ReplyT Int m ()
repl = readLine prompt >>= \case
    ""         -> pure ()
    "quit"     -> quit
    "fail"     -> fail "abcdef"
    "failIO"   -> liftIO $ E.throwIO $ E.userError "Exception thrown in IO."
    "throwM"   -> throwM $ E.userError "Exception thrown in ReplyT."
